...
def oral_absorption_wbpbpk_odes(t, A, tissues, params_parent, kp_dict_parent, met_params_list, kp_dicts_met):
    ...
    for m_idx in range(n_mets):
        ...
        m_start_idx = idx_parent_renal_pass + 1 + m_idx * (n_tissues + 1)
...
def run_animator(drug_name, dose_mg, duration_days, interval_hours, weight_kg, cirrhosis, renal, events_csv, anim_type='body', save_gif=False, drug_csv='benchmark_drugs.csv', met_csv='metabolites.csv'):
    try:
    ...
    total_compartments = (2 * n_gut_comps) + n_tissues + 1 + 1 + n_gut_comps + 3 + n_mets * (n_tissues + 1)
    ...
    if anim_type in ['body', 'both']:
        ...
        feces_patch = patches.Rectangle((-0.4, 0.7), 0.8, 0.6, facecolor='#8b5a2b', edgecolor='black', linewidth=1.2, alpha=0.4)
        ax_body.add_patch(feces_patch)
        ax_body.text(0.0, 1.0, 'Feces', color='black', fontsize=7, fontweight='bold', ha='center', va='center')
        ...
        abs_text = ax_body.text(-3.1, 10.3, "", color='black', fontsize=8, ha='left', va='top', family='monospace')
        ...
        organ_coords = {
            ...
            'arterial_blood': (0.3, 5.8, 0.3, 0.3), 'feces': (0.0, 1.0, 0.7, 0.5)
        }

        patches_dict = {'git': git_oval}
        ax_body.text(0.0, 3.7, 'GIT (ACAT)', color='black', fontsize=7, fontweight='bold', ha='center', va='center')

        for tissue, (x, y, w, h) in organ_coords.items():
            if tissue in ['venous_blood', 'arterial_blood', 'feces']: continue
        ...
        compounds_info = [{'name': f"Parent: {drug_name}", 'color': '#0088cc', 'start_idx': 2 * n_gut_comps, 'exc_idx': (2 * n_gut_comps) + n_tissues}]
        met_colors = ['#cc00cc', '#ff6600', '#009966', '#d62728']
        
        idx_parent_feces = (2 * n_gut_comps) + n_tissues + 1
        idx_parent_abs_start = idx_parent_feces + 1
        idx_biliary = idx_parent_abs_start + 6
        ...
        for idx, m_params in enumerate(met_params_list):
            ...
            compounds_info.append({
                ...
                'color': c, 
                'start_idx': idx_renal_pass + 1 + idx * (n_tissues + 1),
                'exc_idx': idx_renal_pass + 1 + idx * (n_tissues + 1) + n_tissues
            })
        ...
        def update_body(frame):
            git_mass = max(0.0, np.sum(sol_y[0 : 2 * n_gut_comps, frame]))
            total_body_mass = np.sum(sol_y[:, frame]) + 1e-6
            np.random.seed(42 + frame)
            total_particles_per_comp = 200

            feces_val = sol_y[idx_parent_feces, frame]
            stom_val  = sol_y[idx_parent_abs_start, frame]
            duo_val   = sol_y[idx_parent_abs_start+1, frame]
            jej_val   = sol_y[idx_parent_abs_start+2, frame]
            ile_val   = sol_y[idx_parent_abs_start+3, frame]
            cec_val   = sol_y[idx_parent_abs_start+4, frame]
            col_val   = sol_y[idx_parent_abs_start+5, frame]
            bil_val   = sol_y[idx_biliary, frame]
            ren_act   = sol_y[idx_renal_act, frame]
            ren_pas   = sol_y[idx_renal_pass, frame]
            
            dashboard = (
                f"FLUX SUMMARY (mg):\n"
                f"------------------\n"
                f"Stomach: {stom_val:>6.1f}\n"
                f"Duo:     {duo_val:>6.1f}\n"
                f"Jej:     {jej_val:>6.1f}\n"
                f"Ile:     {ile_val:>6.1f}\n"
                f"Col:     {col_val:>6.1f}\n"
                f"Feces:   {feces_val:>6.1f}\n"
                f"------------------\n"
                f"Biliary: {bil_val:>6.1f}\n"
                f"Ren(Act):{ren_act:>6.1f}\n"
                f"Ren(Pas):{ren_pas:>6.1f}\n"
            )
            abs_text.set_text(dashboard)
            ...
            for c_idx, comp in enumerate(compounds_info):
                ...
                feces_mass = max(0.0, sol_y[idx_parent_feces, frame]) if c_idx == 0 else 0.0
                
                comp_total_mass = np.sum(comp_tissue_masses) + (git_mass if c_idx == 0 else 0) + exc_mass + feces_mass + 1e-6
                ...
                if c_idx == 0 and feces_mass > 0:
                    feces_pts = int((feces_mass / comp_total_mass) * total_particles_per_comp)
                    if feces_pts > 0:
                        f_coords = organ_coords['feces']
                        px = np.random.uniform(f_coords[0] - f_coords[2]/2, f_coords[0] + f_coords[2]/2, feces_pts)
                        py = np.random.uniform(f_coords[1] - f_coords[3]/2, f_coords[1] + f_coords[3]/2, feces_pts)
                        particle_x.extend(px); particle_y.extend(py)
    ...
    if anim_type in ['pk', 'both']:
        ...
        lines_pk = []
        line_p, = ax_pk.plot([], [], label=f'Venous Plasma (Parent)', color='#0088cc', lw=2)
        ...
        for m_idx, m_params in enumerate(met_params_list):
            m_venous_idx = idx_renal_pass + 1 + m_idx * (n_tissues + 1) + tissues.index('venous_blood')
            ...
            line_m, = ax_pk.plot([], [], label=f"Venous Plasma ({m_params['Drug']})", color=c, lw=2, linestyle='-')
            lines_pk.append((line_m, np.maximum(m_plasma_conc, 1e-6)))

        ax_pk.axhline(target_threshold, color='red', linestyle='--', label='1/100th Parent Cmax')
        ...
        def update_pk(frame):
            ...
            ax_pk.set_title(f'PK Profile (Phase 4: QSP PopPK)\nTime: {sol_t[frame]:.1f} hours')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Anatomical PBPK ADME Animator (Phase 4)")
    ...
    parser.add_argument('--anim_type', type=str, choices=['body', 'pk', 'both'], default='body')
    ...
    run_animator(args.drug, args.dose, args.duration_days, args.interval_hours, 
                 args.weight, args.cirrhosis, args.renal, args.events_csv,
                 args.anim_type, args.save_gif, args.drug_csv, args.met_csv)